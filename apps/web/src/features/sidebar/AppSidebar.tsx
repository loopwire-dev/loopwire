...
export function AppSidebar() {
	...
	useEffect(() => {
		const handlePointerDown = (event: MouseEvent) => {
			...
			if (
				!target?.closest("[data-workspace-menu-container='true']") &&
				!target?.closest("[data-workspace-menu='true']")
			) {
	...
	const activateWorkspace = (path: string) => {
		...
		if (existing.length === 1) {
			...
			attachWorkspaceSession(path, null, root?.id);
		}
	};
	...
	const isHoveringInteractiveSidebarElement = (
		...
		return element.closest(COMPACT_SIDEBAR_INTERACTIVE_SELECTOR) !== null;
	};
