...
vi.mock("../../../shared/lib/icon-masking", () => ({
	isDataImageUrl: () => false,
	isThemeMaskDisabled: () => false,
	stripMaskMetadata: (value: string) => value,
...
describe("SessionIconPickerDialog", () => {
	beforeEach(() => {
		vi.resetModules();
		...
		DialogMock.mockImplementation(({ children }) => children);
	});

	it("clears and confirms icon draft", async () => {
		...
		const { SessionIconPickerDialog } = await import(
			"../components/SessionIconPickerDialog"
		);
		const tree = SessionIconPickerDialog({
			open: true,
		...
		const clearBtn = findButton(tree, "Clear");
		const confirmBtn = findButton(tree, "Confirm");
		expect(clearBtn).toBeTruthy();
		expect(confirmBtn).toBeTruthy();
		if (!clearBtn || !confirmBtn) throw new Error("Expected dialog actions");
	...
	it("resets crop state and closes when dialog closes", async () => {
