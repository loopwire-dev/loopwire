...
function focusRelativeTreeNode(current: HTMLButtonElement, delta: number) {
  ...
  const currentIndex = nodes.findIndex((node) => node.dataset.path === current.dataset.path);
...
function focusTreeNodeByPath(current: HTMLButtonElement, targetPath: string | null) {
...
export function FileTreeNode({
  ...
  const colorClass = ignored ? "text-[#9e9e9e]" : getStatusColorClass(statusForColor);
  ...
  return (
    <div>
      ...
      >
        ...
        {fileInfo && (fileInfo.additions != null || fileInfo.deletions != null) && (
