...
type FileStatus = "added" | "modified" | "deleted" | "untracked" | "renamed" | "ignored" | "clean";
...
export function useGitStatus(workspaceId: string | null): GitStatusMap {
  ...
  useEffect(() => {
    ...
    const offStatus = wsClient.on("git:status", (envelope) => {
      ...
      const payload = envelope.payload as unknown as GitStatusApiResponse & { workspace_id: string };
    ...
    return () => {
      ...
      offStatus();
      offReconnect();
      wsClient.send("git:unsubscribe", { workspace_id: workspaceId }, { queueWhenDisconnected: false });
    };
  }, [workspaceId]);
