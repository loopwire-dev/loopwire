...
import { fetchGitDiffFiles } from "../../editor/lib/diffUtils";
import { type DirEntry, useFileSystem } from "../hooks/useFileSystem";
				...
				if (imageMimeType) {
					if (file.is_binary) {
						...
						setOpenFile(
							path,
							null,
							`data:${imageMimeType};base64,${file.binary_content_base64}`,
						);
						return;
					}
					if (imageMimeType === "image/svg+xml") {
						...
						setOpenFile(
							path,
							null,
							`data:${imageMimeType};charset=utf-8,${encoded}`,
						);
						return;
					}
				}
		...
		<div className="h-full flex flex-col">
			...
			>
				...
				{!loading && entries.length === 0 && (
					<div className="px-4 py-3 text-xs text-muted">No files found.</div>
				)}
			</div>
		</div>
	);
