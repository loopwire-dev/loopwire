...
import { binaryStringToBytes } from "../channel/TerminalPagingController";
...
export function ScrollbackOverlay({
	...
	const { pages, loading, hasMore, error, fetchInitial, fetchMore, reset } =
		useScrollback();
	const canLoadOlder =
		hasMore && pages.length > 0 && pages[0]?.has_more === true;
	...
	const handleWheel = useCallback(
		(e: React.WheelEvent) => {
			...
			const decision = decideScrollbackWheelAction({
				deltaY: e.deltaY,
				hasMore,
				loading,
				viewportScrollTop: vp.scrollTop,
				latchActive: topWheelLatchRef.current,
			});
			topWheelLatchRef.current = decision.nextLatch;

			if (decision.shouldCheckDismiss) {
				requestAnimationFrame(() => {
					const viewport =
						termRef.current?.element?.querySelector<HTMLElement>(
							".xterm-viewport",
						);
					if (!viewport) return;
