...
// We test the pure functions from useTerminalChannel by reimplementing them here
...
describe("parseOutputFrame", () => {
  it("parses valid history frame", () => {
    ...
    const frame = buildFrame(1, TERM_FRAME_HISTORY, NIL_UUID_BYTES, 0n, payload);
  ...
  it("rejects frame with unknown kind code", () => {
    const frame = buildFrame(1, 3, NIL_UUID_BYTES, 7n, new Uint8Array([10, 20]));
    const result = parseOutputFrame(frame);
  ...
  it("rejects wrong version", () => {
    const frame = buildFrame(99, TERM_FRAME_LIVE, NIL_UUID_BYTES, 0n, new Uint8Array(0));
    expect(parseOutputFrame(frame)).toBeNull();
  });
  ...
  it("rejects frame with payload length exceeding buffer", () => {
    const frame = buildFrame(1, TERM_FRAME_LIVE, NIL_UUID_BYTES, 0n, new Uint8Array(5));
  ...
  it("parses frame with empty payload", () => {
    const frame = buildFrame(1, TERM_FRAME_LIVE, NIL_UUID_BYTES, 0n, new Uint8Array(0));
    const result = parseOutputFrame(frame);
    expect(result).not.toBeNull();
...
describe("formatUuid", () => {
  it("formats nil UUID", () => {
    expect(formatUuid(NIL_UUID_BYTES)).toBe("00000000-0000-0000-0000-000000000000");
  });

  it("formats a known UUID", () => {
    expect(formatUuid(TEST_UUID_BYTES)).toBe("12345678-1234-1234-1234-123456789abc");
  });
