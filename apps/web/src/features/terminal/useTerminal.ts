...
import { TerminalChannel } from "./useTerminalChannel";
import {
...
export function useTerminal(
  ...
  const pagingControllerRef = useRef<TerminalPagingController>(new TerminalPagingController());
  ...
  useEffect(() => {
    ...
    term.attachCustomWheelEventHandler((event) => {
      ...
      if (vp.scrollHeight > vp.clientHeight) {
        ...
        const lineHeightScale = term.options.lineHeight ?? 1;
        const lineHeight = Math.ceil(fontSize * lineHeightScale);
        let deltaPx: number;
        if (event.deltaMode === 1) {
          deltaPx = event.deltaY * lineHeight;
        } else if (event.deltaMode === 2) {
          deltaPx = event.deltaY * vp.clientHeight;
        } else {
          deltaPx = event.deltaY;
        }

        if (vp.scrollTop === 0 && deltaPx < 0) {
          onScrollPastTopRef.current?.();
        } else if (deltaPx !== 0) {
          vp.scrollTop += deltaPx;
        }
      ...
      handlers: {
        ...
        onReady: ({ sessionId: readySessionId }) => {
          ...
          if (terminal.cols > 0 && terminal.rows > 0) {
            channel.sendResize(terminal.cols, terminal.rows);
          }
        },
        onOutput: (meta, kind, bytes) => {
