...
export function useAgent() {
  ...
  const startSession = useCallback(
    async (agentType: string, workspacePath: string) => {
      ...
      try {
        ...
        setActivePanel(workspacePath, { kind: "agent", sessionId: res.session_id });
    ...
    [attachWorkspaceSession, setActivePanel, setWorkspace, upsertWorkspaceSession],
  );
  ...
  const updateSessionSettings = useCallback(
    async (sessionId: string, settings: { pinned?: boolean; icon?: string | null; sort_order?: number | null }) => {
