...
import { useGitGutter } from "./useGitGutter";
...
export function CodeEditor() {
  ...
  useGitGutter(workspaceId, gutterFilePath, editorRef, monacoRef, isDark, editorReady);
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between border-b border-border bg-surface-raised px-4 py-3">
        <p className="text-sm font-mono">
          <span className="text-muted">{filePath.substring(0, filePath.lastIndexOf("/") + 1)}</span>
        ...
        <div className="flex items-center gap-2">
          {isMarkdown && (
            ...
            >
              {previewMode ? <Code aria-hidden="true" size={14} /> : <Eye aria-hidden="true" size={14} />}
            </button>
          )}
          <button
            type="button"
      ...
      <div className="flex-1 overflow-hidden">
        {imageSrc ? (
          ...
          <div className={`h-full overflow-auto p-6 ${isDark ? "bg-surface text-foreground" : "bg-white text-gray-900"}`}>
